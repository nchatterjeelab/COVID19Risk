...
# ------- combine
png(paste0('~/Dropbox/NHANES_risk_score/results/highrisk_',threshold_type,'/suppfig7_draft.png'),
...
# ------- combine
png(paste0('~/Dropbox/NHANES_risk_score/results/highrisk_',threshold_type,'/suppfig7_nolabel.png'),
    units='px', width=1800*5, height=1800*2.5, res=170*3,type = "cairo")
...
counts = c(1,1,1,2,5,2,3,6,4,7,15,7,14,21,12,25,17,20,18,20,24,21,19,19,20,19,13,12,12,10,9,9,8,6,8,8,5,4,4,1,1,4,2,1,2)
sum(counts)
...
counts = c(1,1,2,1,1,1,2,1,2,1,3,5,5,4,7,12,10,7,4,4,11,18,15,25,13,26,19,21,17,14,23,22,22,20,21,16,21,12,13,10,6,3)
sum(counts)
cumulative = sapply(1:length(counts),function(x){sum(counts[1:x])})
