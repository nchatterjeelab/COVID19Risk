...
x.breaks = seq(1,1+20*5,by=20) #which(as.character(x.labels) %in% c('Jun 15 ', 'Jul 05 ', 'Jul 25 ', 'Aug 15 ', 'Sep 05 ', 'Sep 25 '))
x.index = x.breaks
...
write.xlsx(output, file=paste0('figures_and_tables/output/model_validation-city.xlsx'))
...
rownames(output) = NULL
output[,-1] = signif(output[,-1],3)
write.xlsx(output, file=paste0('figures_and_tables/output/Model_validation-city_conditional_on_regions.xlsx'))
...
plot1 <- ggplot(data=results, aes(x=Time_period, y=R2, group=interaction(Variable,conditional))) +
  ...
  scale_y_continuous(labels = y.labels, breaks = y.breaks, expand = c(0, 0.005),limits=c(0,0.55))  +
...
plotNew <- plot1 + 
  annotation_custom(grob = leg1, xmin = 20, xmax = 22, ymin = 0.4, ymax = 0.6)
plotNew <- plotNew + 
  annotation_custom(grob = leg2, xmin = 60, xmax = 62, ymin = 0.42, ymax = 0.62)
...
plot2 <- ggplot(data=dr, aes(x=x, y=death)) +
  ...
  removeGrid(y = FALSE) + 
  scale_x_continuous(labels = x.labels[x.breaks], breaks = x.index,expand = c(0, 0))  +
  scale_y_continuous(labels = dr.breaks, breaks = dr.breaks, limits = c(0,1e4),
                     expand = c(0, 0))

png(paste0('figures_and_tables/output/Figure 2.png'),
