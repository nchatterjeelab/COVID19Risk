...
p4 = ggplot(hr,aes(N,fill='Pale Green 4')) +
  ...
  scale_x_continuous(breaks = xaxis.breaks, 
                     labels = xaxis.display) + 
  scale_y_continuous(expand = c(0,0), breaks=seq(0,230,40), limits = c(0,230)) + 
  removeGrid(x = FALSE, y = FALSE) 
...
highrisk.medicare = read.xlsx(paste0('~/Dropbox/NHANES_risk_score/results_updated/tables/highrisk_medicare_among_',scenario[1],'.xlsx'))
highrisk.medicare = as.data.frame(highrisk.medicare)

highrisk.medicare2 = read.xlsx(paste0('~/Dropbox/NHANES_risk_score/results_updated/tables/highrisk_medicare_among_',scenario[2],'.xlsx'))
...
png(paste0('figures_and_tables/output/Figure 3.png'),
