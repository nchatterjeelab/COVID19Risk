...
for (i in 1:M){
  for (j in 1:M){
    if (i!=j){
      ...
      weights = dat %>% dplyr::select(sampling_weights, PPSU, PSTRAT)
...
Rl = 4.595036 # estimated from city-level analyses
...
ier = read.xlsx('data_created/IER-medicare.xlsx')
# ----- State-level average of high risk proportions
highrisk.US.all = data.frame(proportion_k_1.2=0, N_k_1.2=0,
                             proportion_k_2=0, N_k_2=0,
                             proportion_k_5=0, N_k_5=0,
                             proportion_k_10=0, N_k_10=0)
highrisk.US.deaths = data.frame(proportion_k_1.2=0, proportion_k_2=0,
                                proportion_k_5=0, proportion_k_10=0)
highrisk.county.all = read.xlsx('data_created/highrisk_medicare_among_all.xlsx')
for (i in 1:nrow(highrisk.US.all)){
  temp = cbind(dist.rs.medicare,
               ier,highrisk.county.all)
  Ws = temp$population/sum(temp$population)
  mus = temp$mu.rs
  vars = temp$var.rs
  for (k in c(1.2,2,5,10)){
    # high risk among all
    highrisk.US.all[i,paste0('proportion_k_',k)] = sum(Ws * temp[,paste0('Proportion.k=',k)])
    highrisk.US.all[i,paste0('N_k_',k)] = round(highrisk.US.all[i,paste0('proportion_k_',k)] * sum(temp$population),0)
    # high risk among deaths
    Pr.k.above = sum(Ws * sapply(1:nrow(temp),function(x){pnorm(log(k)+log(Rl), mus[x]+vars[x], vars[x], lower.tail = F) * exp(mus[x]+0.5*vars[x])}))
    Pr.k.below = sum(Ws * sapply(1:nrow(temp),function(x){pnorm(log(k)+log(Rl), mus[x]+vars[x], vars[x], lower.tail = T) * exp(mus[x]+0.5*vars[x])}))
    highrisk.US.deaths[i,paste0('proportion_k_',k)] = Pr.k.above/(Pr.k.above + Pr.k.below)
  }
}
write.xlsx(highrisk.US.all,'data_created/highrisk-medicare-among-all-US-average.xlsx')
write.xlsx(highrisk.US.deaths,'data_created/highrisk-medicare-among-deaths-US-average.xlsx')
